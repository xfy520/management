...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
					...
					formatNoMatches: options.formatNoMatches,           // 空数据自定义提示语句
					...
					onRefreshOptions: options.onRefreshOptions			// 刷新配置时执行
                });
            },
        ...
        operate: {
            ...
            edit: function(obj) {
            	table.set();
            	if($.common.isEmpty(obj.id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open("修改" + table.options.modalName, url, obj.w, obj.h);
            	} else {
            	    $.modal.open("修改" + table.options.modalName, $.operate.editUrl(obj.id), obj.w, obj.h);
            	}
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
        	...
        	init: function(options) {
        		...
        		var setting = {
    				callback: {
						...
						onExpand: options.onExpand,						//用于捕获节点展开
						onCollapse: options.onCollapse,
						onDblClick: options.onDblClick,                 // 用于捕获鼠标双击之后的事件回调函数
						onAsyncSuccess: options.onAsyncSuccess			//用于异步加载成功回调,
    			    },
					async: {
						enable: true,
						url: options.url,
				...
				$.fn.zTree.init($("#" + options.id), setting);
        	},
			onAsyncSuccess:function(treeId, treeNode){
				$._tree = $.fn.zTree.getZTreeObj(treeId);
				var nodes = $._tree.getNodesByParam("level", $.tree._option.expandLevel - 1);
				for (var i = 0; i < nodes.length; i++) {
					$._tree.expandNode(nodes[i], true, false, false);
				}
				if (nodes.length>0 && ($.tree._option.expandLevel - 1) >= -1 ) {
					const index = $.tree._option.expandLevel - 1;
					$._tree.selectNode(nodes[index >=0 ? index : 0]);
				}
				var node = $._tree.getNodesByParam("id", $("#treeId").val(), null)[0];
				if(node){
