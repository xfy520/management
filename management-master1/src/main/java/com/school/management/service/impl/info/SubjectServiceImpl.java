...
public class SubjectServiceImpl implements ISubjectService {
    ...
    {
        Long maxSubjectNumber = subjectMapper.maxSubjectNumber();
        if (maxSubjectNumber == null){
            maxSubjectNumber = 100000000L;
        }else{
            maxSubjectNumber = maxSubjectNumber + 1L;
        }
        subject.setSubjectId(IDUtils.getId());
        subject.setSubjectNumber(maxSubjectNumber);
    ...
    {
        String[] subjectIds = Convert.toStrArray(ids);
     ...
     * 查询考试科目
     */
    public List<Subject> selectExamineSubjectList(String examId){
        return subjectMapper.selectExamineSubjectList(examId);
    }
}
